 ...
 */
export function compilePolicyToSql(source: string, model: ModelDefinition, allModels: ModelDefinition[]): string {
...
function resolveChainToSql(chain: string[], model: ModelDefinition, allModels: ModelDefinition[], scope: Scope): string {
  ...
  if (rootName === 'user') {
    if (chain.length === 2) {
      const prop = chain[1];
      switch (prop) {
        case 'id': return "laforge_user_id()";
        case 'tenantId': return "laforge_tenant_id()";
        case 'role': return "laforge_user_role()";
  ...
  if (firstTenant) {
    whereClause += ` AND ${firstAlias}.${toSnakeCase(firstTenant[0])} = laforge_tenant_id()`;
  }
  ...
  for (let i = 2; i < chain.length - 1; i++) {
    ...
    if (tenant) {
      from += ` AND ${targetAlias}.${toSnakeCase(tenant[0])} = laforge_tenant_id()`;
    }
...
function resolveCollectionPredicate(node: t.CallExpression, model: ModelDefinition, allModels: ModelDefinition[], scope: Scope): string {
  ...
  if (firstTenant) {
    subqueryWhere += ` AND ${firstAlias}.${toSnakeCase(firstTenant[0])} = laforge_tenant_id()`;
  }
  ...
  for (let i = 2; i < chain.length; i++) {
    ...
    if (tenant) {
      subqueryFrom += ` AND ${targetAlias}.${toSnakeCase(tenant[0])} = laforge_tenant_id()`;
    }
...
CREATE OR REPLACE FUNCTION laforge_user_id() RETURNS uuid AS $$
  SELECT NULLIF(current_setting('laforge.user_id', true), '')::uuid;
$$ LANGUAGE sql STABLE;

CREATE OR REPLACE FUNCTION laforge_tenant_id() RETURNS uuid AS $$
  SELECT NULLIF(current_setting('laforge.tenant_id', true), '')::uuid;
$$ LANGUAGE sql STABLE;

CREATE OR REPLACE FUNCTION laforge_roles() RETURNS text[] AS $$
  SELECT COALESCE(string_to_array(current_setting('laforge.roles', true), ','), ARRAY[]::text[]);
$$ LANGUAGE sql STABLE;

CREATE OR REPLACE FUNCTION laforge_claims() RETURNS text[] AS $$
  SELECT COALESCE(string_to_array(current_setting('laforge.claims', true), ','), ARRAY[]::text[]);
$$ LANGUAGE sql STABLE;

CREATE OR REPLACE FUNCTION laforge_user_role() RETURNS text AS $$
  SELECT COALESCE(current_setting('laforge.user_role', true), (laforge_roles())[1]);
$$ LANGUAGE sql STABLE;

CREATE OR REPLACE FUNCTION laforge_has_any_role(required text[]) RETURNS boolean AS $$
  SELECT cardinality(array_remove(required, NULL)) = 0 OR EXISTS (SELECT 1 FROM unnest(required) r WHERE r = ANY(laforge_roles()));
$$ LANGUAGE sql STABLE;

CREATE OR REPLACE FUNCTION laforge_has_any_claim(required text[]) RETURNS boolean AS $$
  SELECT cardinality(array_remove(required, NULL)) = 0 OR EXISTS (SELECT 1 FROM unnest(required) c WHERE c = ANY(laforge_claims()));
$$ LANGUAGE sql STABLE;
`;

  const buildRbacPredicate = (action: 'create' | 'read' | 'update' | 'delete', model: ModelDefinition): string | null => {
    const compiledRule = model.rbacSpec?.actions?.[action] as any;
    const rawRule = compiledRule || (model.permissions as any)?.[action];
    if (!rawRule) return null;

    const roles = (compiledRule ? compiledRule.roles : rawRule.roles) ?? [];
    const claims = (compiledRule ? compiledRule.claims : rawRule.claims) ?? [];
    const abac = compiledRule?.abacSql ?? (rawRule.condition ? compilePolicyToSql(rawRule.condition, model, models) : '');

    const rolesArray = roles.length ? roles.map((r: string) => `'${r.replace(/'/g, "''")}'`).join(', ') : '';
    const claimsArray = claims.length ? claims.map((c: string) => `'${c.replace(/'/g, "''")}'`).join(', ') : '';
    const roleCheck = rolesArray ? `laforge_has_any_role(ARRAY[${rolesArray}])` : '';
    const claimCheck = claimsArray ? `laforge_has_any_claim(ARRAY[${claimsArray}])` : '';
    const baseChecks = [roleCheck, claimCheck].filter(Boolean);
    const base = baseChecks.length ? baseChecks.map(c => `(${c})`).join(' OR ') : 'TRUE';
    const baseWrapped = baseChecks.length ? `(${base})` : 'TRUE';
    if (abac) {
      return `(${baseWrapped} AND (${abac}))`;
    }
    return `(${baseWrapped})`;
  };
  ...
  for (const model of models) {
    ...
    if (config.multiTenant && tenantColumn) {
      tenantPolicy = `${tenantColumn} = laforge_tenant_id()`;
    }

    const readPolicy = model.policies.read;
    const readRbac = buildRbacPredicate('read', model);
    if (readPolicy) {
      const usingExpression = compilePolicyToSql(readPolicy.handlerSource, model, models);
      const clauses = [tenantPolicy, usingExpression, readRbac].filter(Boolean);
      const finalExpression = clauses.length ? `(${clauses.join(') AND (')})` : 'TRUE';
      rlsSql += `CREATE POLICY forge_select_${tableName} ON ${tableRef} FOR SELECT USING ${finalExpression};\n`;
    } else if (tenantPolicy || readRbac) {
      const clauses = [tenantPolicy, readRbac].filter(Boolean);
      const finalExpression = clauses.length ? `(${clauses.join(') AND (')})` : 'TRUE';
      rlsSql += `CREATE POLICY forge_select_${tableName} ON ${tableRef} FOR SELECT USING ${finalExpression};\n`;
    }

    const createPolicy = model.policies.create;
    const createRbac = buildRbacPredicate('create', model);
    ...
    if (createPolicy) {
      ...
      const clauses = [tenantPolicy, createExpression, createRbac].filter(Boolean);
      withCheckExpression = clauses.length ? `(${clauses.join(') AND (')})` : 'TRUE';
    } else if (createRbac) {
      const clauses = [tenantPolicy, createRbac].filter(Boolean);
      withCheckExpression = `(${clauses.join(') AND (')})`;
    }
